...
uploadFile = (body)  => {
    return new Promise((resolve, reject) =>{
        bucket.upload(body.path, {destination: bucket.file(`profilePhoto/${(new Date().getTime()/1000)}.png`)}, function(err, file, apiResponse) {
            obj = {
                status: 200,
...
deleteFile = (body)  => {
    return new Promise((resolve, reject) =>{
        let file = bucket.file(`profilePhoto/${body.file}`);
